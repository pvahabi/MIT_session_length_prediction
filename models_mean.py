...
def baseline(indexes_listeners_train, indexes_listeners_val, indexes_listeners_test, y_data_users_train, params_train_df, y_data_users_val, y_data_users_test):
	...
	for i in range(len(indexes_listeners_test)):  
		...
		sum_error_user     = np.sum([abs(baseline_train - y_data) for y_data in y_data_user_val])
		baseline_val_MAE  += sum_error_user

		y_data_user_test   = y_data_users_test[ indexes_listeners_test[i] ]
		sum_error_user     = np.sum([abs(baseline_train - y_data) for y_data in y_data_user_test])
		baseline_test_MAE += sum_error_user
...
def baseline_log(indexes_listeners_train, indexes_listeners_val, indexes_listeners_test, y_log_data_users_train, params_train_df, y_data_users_val, y_data_users_test, dict_baseline_pairs_sessions_errors):
	...
	log_baseline_val_MAE  = 0
	log_baseline_test_MAE = 0
	...
	for i in range(len(indexes_listeners_test)):  
		...
		y_data_user_val         = y_data_users_val[ indexes_listeners_val[i] ]
		sum_error_user          = np.sum([abs(math.exp(log_baseline_train) - y_data) for y_data in y_data_user_val])
		log_baseline_val_MAE  += sum_error_user

		y_data_user_test        = y_data_users_test[ indexes_listeners_test[i] ]
		sum_error_user          = np.sum([abs(math.exp(log_baseline_train) - y_data) for y_data in y_data_user_test])
		log_baseline_test_MAE  += sum_error_user
...
def ridge(indexes_listeners_train, indexes_listeners_val, indexes_listeners_test, x_data_users_train, x_train_columns, y_log_data_users_train, params_train_df, x_data_users_val, y_data_users_val, x_data_users_test, y_data_users_test, dict_baseline_pairs_sessions_errors, f, baseline_val=1, baseline_test=1, theoretical_alpha=False, is_test_standardized=False):
	...
	### Test results
		ratio_cov_EB_val_MAE       = 0
		ratio_cov_EB_test_MAE      = 0
		dict_pairs_sessions_errors = {}
		...
		### Data user
			...
			if n_user_val>0:
				if not is_test_standardized:
					...
					sum_error_user = np.sum(np.abs( np.exp(np.dot(x_data_user_val, beta_train) + add*ones_N[:n_user_val] ) - y_data_user_val ))
				else:
					...
					sum_error_user = np.sum(np.abs( np.exp(pred_EB_val) - y_data_user_val ))
			else:
				sum_error_user = 0

			ratio_cov_EB_val_MAE += sum_error_user
		...
		### Data user
			...
			if n_user_test>0:
				if not is_test_standardized:
					add = params_train_df['log_session_duration']['mean'] - np.dot(all_means_x_data_users_train, beta_train) 
					sum_error_user = np.sum(np.abs( np.exp(np.dot(x_data_user_test, beta_train) + add*ones_N[:n_user_test] ) - y_data_user_test ))
				else:
					...
					sum_error_user = np.sum(np.abs( np.exp(pred_EB_test) - y_data_user_test ))
			else:
				sum_error_user = 0

			
			n_user_train = len(y_log_data_users_train[indexes_listeners_train[i]])
			dict_pairs_sessions_errors[n_user_train] =  sum_error_user if n_user_train not in dict_pairs_sessions_errors.keys() else dict_pairs_sessions_errors[n_user_train]+sum_error_user
			...
			ratio_cov_EB_test_MAE += sum_error_user
...
def EB_log_towards_mean(list_llambda, indexes_listeners_train, indexes_listeners_val, indexes_listeners_test, y_log_data_users_train, number_points_users_train, params_train_df, y_data_users_val, y_data_users_test, dict_baseline_pairs_sessions_errors, baseline_val=1, baseline_test=1, theoretical_alpha=False):
	...
	for llambda in list_llambda:
		...
		### Prediction
			...
			y_data_user_val =  y_data_users_val[ indexes_listeners_val[i] ]
			sum_error_user  = np.sum([abs(math.exp(EB_train) - y_data)  for y_data in y_data_user_val])
			EB_val_MAE     += sum_error_user

			y_data_user_test =  y_data_users_test[ indexes_listeners_test[i] ]
			sum_error_user   = np.sum([abs(math.exp(EB_train) - y_data) for y_data in y_data_user_test])
			EB_test_MAE     += sum_error_user

			dict_pairs_sessions_errors[number_points_user_train] =  sum_error_user if number_points_user_train not in dict_pairs_sessions_errors.keys() else dict_pairs_sessions_errors[number_points_user_train]+sum_error_user
			#dict_pairs_sessions_errors[number_points_user_train] =  sum_error_user/(1e-10 + dict_baseline_pairs_sessions_errors[number_points_user_train]) if number_points_user_train not in dict_pairs_sessions_errors.keys() else dict_pairs_sessions_errors[number_points_user_train]+sum_error_user/(1e-10 + dict_baseline_pairs_sessions_errors[number_points_user_train])
...
def loop_alternative_minimization(list_llambda, indexes_listeners_train, indexes_listeners_val, indexes_listeners_test, x_data_users_train, x_train_columns, y_log_data_users_train, number_points_user_train, params_train_df, x_data_users_val,  y_data_users_val, x_data_users_test,  y_data_users_test, dict_baseline_pairs_sessions_errors, f, baseline_val=1, baseline_test=1, penalization='l2', use_L0=False, range_K=[0], is_test_standardized=False):
	...
	list_alpha 	   = [alpha_max*0.7**i for i in range(N_ALPHA)][10:]
...
######################## TRY L0 ON XGBOOST FEATURES ########################
				...
				######### VAL #######
					...
					if n_user_val>0:	
						if not is_test_standardized:
							...
							sum_error_user  = np.sum(np.abs( np.exp(np.dot(x_data_user_val, beta_train) + pred_EB_val*ones_N[:n_user_val] ) - y_data_user_val ))
						else:
							pred_EB_val     = params_train_df['log_session_duration']['std'] * (EB_residual_users[i] + np.dot(x_data_user_val, beta_train_std)) + params_train_df['log_session_duration']['mean'] 
							sum_error_user  = np.sum(np.abs( np.exp(pred_EB_val) - y_data_user_val ))
					else:
						sum_error_user = 0
					ratio_cov_EB_val_MAE += sum_error_user
				...
				######### TEST #######
					...
					if n_user_test>0:
						...
						if not is_test_standardized:
							...
							sum_error_user  = np.sum(np.abs( np.exp(np.dot(x_data_user_test, beta_train) + cov_EB_test*ones_N[:n_user_test] ) - y_data_user_test ))
						else:
							pred_EB_test    = params_train_df['log_session_duration']['std'] * (EB_residual_users[i] + np.dot(x_data_user_test, beta_train_std)) + params_train_df['log_session_duration']['mean'] 
							sum_error_user  = np.sum(np.abs( np.exp(pred_EB_test) - y_data_user_test ))
					else:
						sum_error_user = 0

					n_user = len(y_log_data_users_train[ indexes_listeners_train[i] ])
					dict_pairs_sessions_errors[n_user] =  sum_error_user if n_user not in dict_pairs_sessions_errors.keys() else dict_pairs_sessions_errors[n_user]+sum_error_user
					#dict_pairs_sessions_errors[n_user] =  sum_error_user/(1e-10 + dict_baseline_pairs_sessions_errors[n_user]) if n_user not in dict_pairs_sessions_errors.keys() else dict_pairs_sessions_errors[n_user]+sum_error_user/(1e-10 + dict_baseline_pairs_sessions_errors[n_user])
					ratio_cov_EB_test_MAE += sum_error_user
	...
	dict_best = {'alpha': best_params[0], 'lambda': best_params[1], 'beta': np.array(best_beta_std), 'error':best_ratio_test, 'sessions_errors': dict_best_pairs_sessions_errors}
	return all_ratios_val, all_ratios_test, dict_best
...
# y_data_users_test    : for test accuracy
	...
	for params in params_grid:
		...
		### Compute the test accuracy -> the data is standardized !!
			...
			######### VAL #######
				...
				if n_user_val>0:
					xgb_val_predictions   = xgb_model.predict(np.array(x_data_user_val))
					xgb_val_predictions   = [params_train_df['log_session_duration']['mean'] + params_train_df['log_session_duration']['std']*(pred + EB_residual_users[i]) for pred in xgb_val_predictions]
					sum_error_user        = np.sum(np.abs( np.exp(xgb_val_predictions) - y_data_user_val ) )
				else:
					sum_error_user = 0
				ratio_cov_EB_val_MAE += sum_error_user
			...
			######### TEST #######
				...
				if n_user_test>0:
					xgb_test_predictions   = xgb_model.predict(np.array(x_data_user_test))
					xgb_test_predictions   = [params_train_df['log_session_duration']['mean'] + params_train_df['log_session_duration']['std']*(pred + EB_residual_users[i]) for pred in xgb_test_predictions]
					sum_error_user 		   = np.sum(np.abs( np.exp(xgb_test_predictions) - y_data_user_test ) )
				else:
					sum_error_user = 0
	...
	dict_best = {'params': best_params, 'model': best_model, 'error': best_ratio_test, 'sessions_errors': dict_best_pairs_sessions_errors}
	return all_ratios_val, all_ratios_test, dict_best
...
def xgboost(indexes_listeners_train, indexes_listeners_val, indexes_listeners_test, x_data_users_train, y_data_users_train, x_data_users_val_STD,  y_data_users_val, x_data_users_test_STD,  y_data_users_test, params_train_df, f, baseline_val=1, baseline_test=1, predict_log=False, only_size_2=False):
	...
	max_depth    = [6,10] if not only_size_2 else [2]
	n_estimators = [10, 15, 50, 100] if not only_size_2 else [50]
	...
	### Data not standardized
		...
		if not predict_log:
			xgb_all_user_val_MAE          = np.linalg.norm(xgb_all_user_predictions_val  - y_data_users_val, 1)
			xgb_all_user_test_MAE         = np.linalg.norm(xgb_all_user_predictions_test - y_data_users_test, 1)
		else:
			...
			xgb_all_user_val_MAE         = np.linalg.norm(np.exp(xgb_all_user_val_predictions)  - y_data_users_val, 1)
			...
			xgb_all_user_test_MAE         = np.linalg.norm(np.exp(xgb_all_user_test_predictions) - y_data_users_test, 1)
	...
	######### ADD FOR PLOT #######
		...
		for i in range(len(indexes_listeners_test)): 
			...
			x_data_user_test = x_data_users_test_STD[ indexes_listeners_test[i] ]
			y_data_user_test = y_data_users_test[ indexes_listeners_test[i] ]
			n_user_test      = x_data_user_test.shape[0]
			#print n_user_test

			if n_user_test>0:
				sum_error_user = np.sum(np.abs( xgb_model.predict(np.array(x_data_user_test)) - y_data_user_test ) )
			else:
				sum_error_user = 0

			n_user = len(y_data_users_train[ indexes_listeners_train[i] ])
			dict_pairs_sessions_errors[n_user] =  sum_error_user if n_user not in dict_pairs_sessions_errors.keys() else dict_pairs_sessions_errors[n_user]+sum_error_user
		...
		write_and_print('\nTime xgboost: '+str(time.time() - start_time), f)
		write_and_print('Val error : '+str(xgb_all_user_val_MAE/baseline_val), f)
		write_and_print('Test error: '+str(xgb_all_user_test_MAE/baseline_test), f)


	argmin    = np.argmin(all_ratios_val)    
	dict_best = {'params': params_grid[argmin], 'model': all_models_str[argmin], 'error': all_ratios_test[argmin], 'sessions_errors': all_dict_pairs_sessions_errors[argmin]}
	return all_ratios_val, all_ratios_test, dict_best
